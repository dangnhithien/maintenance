...
const PartDetailList: React.FC = () => {
  ...
  const { partDetails, fetchPartDetails, deletePartDetail } = usePartDetail();
  ...
  return (
    <div>
      ...
      <Grid2 container spacing={2}>
        {partDetails.map((device) => (
          <Grid2 key={device.id} size={{ xs: 12, sm: 6, md: 4, xl: 4 }}>
            <DeviceCard
              linkBaseUrl="/device-Details/update"
              deviceId={device.id}
              image={
