...
import usePartGroup from "@modules/maintenance/hooks/usePartGroup";
...
const PartGroupList: React.FC = () => {
  const [openPopupHardDelete, setOpenPopupHardDelete] = useState(false);
  const { notify } = useNotification();

  const [params, setParams] = useState<IPartGroupGet>({
    takeCount: 10,
    sortBy: "CreatedDate DESC",
  });

  const { partGroups, fetchPartGroups, deletePartGroup } = usePartGroup();

  const initData = () => {
    fetchPartGroups(params);
  };
  ...
  return (
    <div>
      ...
      <Grid2 container spacing={2}>
        {partGroups.map((device) => (
          <Grid2 key={device.id} size={{ xs: 12, sm: 6, md: 4, xl: 4 }}>
            <DeviceCard
              linkBaseUrl="/device-Groups/update"
              deviceId={device.id}
              image={
