...
import usePartSKU from "@modules/maintenance/hooks/usePartSKU";
...
const PartSKUList: React.FC = () => {
  const [openPopupHardDelete, setOpenPopupHardDelete] = useState(false);
  const { notify } = useNotification();

  const [params, setParams] = useState<IPartSKUGet>({
    takeCount: 10,
    sortBy: "CreatedDate DESC",
  });

  const { partSKUs, fetchPartSKUs, deletePartSKU } = usePartSKU();

  const initData = () => {
    fetchPartSKUs(params);
  };
